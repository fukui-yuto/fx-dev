...
**自動チューニング**（IS/OOS 検証）で最良戦略を自動選択し、リアルタイムのエントリー・エグジットタイミングと具体的な Entry / SL / TP 価格をチャート上に表示します。
...
| カテゴリ | 機能 |
|---|---|
| **自動チューニング** | IS/OOS 分割検証で 5 戦略×32 パラメータを自動最適化、24 時間キャッシュ |
| **ライブシグナル** | 最良戦略に基づくリアルタイム BUY/SELL/WAIT 判定、Entry / SL / TP 価格を表示 |
| **エントリー・エグジット表示** | バックテスト結果をチャートにマーカー表示（▲BUY / ▼SELL / ◎TP / ✕SL / →EXIT） |
...
- **自動チューニングマーカー** — IS/OOS 検証済み最良戦略のエントリー（▲/▼）・エグジット（◎TP / ✕SL / →EXIT）をチャートに常時表示
- **🎯 ライブシグナルタブ** — 5 秒ごとに BUY/SELL/WAIT を判定し、具体的な Entry / SL / TP 価格・pips・信頼度・HTF 整合性を表示
- **信号ライン** — Entry（黄破線）/ SL（赤）/ TP（緑）の水平ラインをチャートに表示
...
1. 直近データを **IS（最適化期間）70%** と **OOS（検証期間）30%** に分割
2. IS 期間で 5 戦略 × 各パラメータグリッドを全探索
3. OOS 期間で上位候補を再評価し、**OOS PF（利益係数）** が最も高い戦略・パラメータを選択
...
| 戦略名 | 概要 |
|---|---|
| EMA クロス | 短期・長期 EMA のクロス。パラメータ: 9 通り |
| ドンチャンブレイクアウト | N 本高値 / 安値のブレイクアウト。パラメータ: 3 通り |
| トリプル確認（EMA+RSI+MACD） | 3 指標同時一致。パラメータ: 4 通り |
| RSI × BB | BB バウンス + RSI 反転。パラメータ: 8 通り |
| 夜間スカルパー（4 重確認） | 夜間時間帯の多重フィルター。パラメータ: 8 通り |
...
- **HTF 整合性**: 上位時間足の EMA200 方向とシグナル方向が一致しているか確認
- **SL/TP**: ATR×1.5 を SL 幅、SL×2.0 を TP 幅（RR 1:2）として自動計算
...
│   │
│   ├── auto_tuner.py               # IS/OOS 自動チューニングエンジン（24h キャッシュ）
...
│
├── output/                         # 自動チューニング結果・バックテスト結果（.gitignore）
